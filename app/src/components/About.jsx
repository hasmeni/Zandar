...
const AboutPage = ({ cardDismissal, setCardDismissal, started }) => {
  ...
  const handleBacktoDashboard = () => {
    if (started == false) {
      localStorage.removeItem("cardDismissal");
  ...
  return (
    <div
      className={`min-h-screen flex justify-center p-6 md:items-center overflow-y-auto font-sans relative transition-colors duration-500 ease-in-out ${theme.container}`}
    >
      {/* --- NAVIGATION CONTROLS --- */}
      ...
      <div
        ...
        className={`absolute top-4 left-4 md:top-6 md:left-6 flex items-center gap-2 py-2 px-3 rounded-lg text-sm font-medium transition-all group ${theme.backBtn}`}
      >
        <ArrowLeft
          size={16}
      ...
      <div className="w-full max-w-2xl space-y-12 mt-12 md:mt-0">
        ...
        <div className="space-y-4 text-center">
          <div
            className={`inline-flex items-center gap-2 px-3 py-2 rounded-full border text-xs font-semibold transition-colors duration-300 ${theme.badge}`}
          >
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
            ...
            <a
              href="#"
              className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-colors border ${isDarkMode ? "bg-indigo-500/10 text-indigo-400 border-indigo-500/20 hover:bg-indigo-500/20" : "bg-indigo-600/10 text-indigo-900 border-indigo-700/10 hover:bg-indigo-600/20"}`}
            >
              <Zap size={16} />
              Request Feature
            </a>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            <div
              className={`flex items-center gap-2 d text-xl transition-colors duration-300 ${theme.textMain}`}
            >
            ...
            <ul className="space-y-4">
              ...
              <FeatureItem
                ...
                desc="Curated aesthetic controls."
              />
              <FeatureItem
                theme={theme}
                title="Import / Export"
                desc="Backup your setup in JSON."
              />
            </ul>
          </div>
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-4">
              <RoadmapItem
                ...
                desc="Save links to Zandar instantly & Full new-tab integration"
              />
              <RoadmapItem
                theme={theme}
                title="Cloud Sync"
                desc="Optional encrypted sync."
              />
              <RoadmapItem
                theme={theme}
...
const ActionButton = ({ href, icon: Icon, label, theme }) => (
  <a
    href={href}
    className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-colors border border-transparent ${theme.cardBg} ${theme.textAccent} hover:border-current`}
  >
    <Icon size={16} />
    {label}
  </a>
...
const FeatureItem = ({ title, desc, theme }) => (
  <li className="group flex flex-col gap-0.5">
    <span
      className={`text-sm font-bold transition-colors duration-300 group-hover:text-emerald-500 ${theme.textMain}`}
    >
      {title}
    </span>
    <span
